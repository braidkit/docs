...
The existing Braid target-architecture diagram is embedded below.
...
1. **Local Dev Box** - captures agent work and remains the working source of truth before a pull request exists.
2. **Remote Server** - stores the durable intent and lifecycle record after the pull-request boundary.
3. **GitHub** - hosts pull requests, merges, code refs, and pushed Git-note refs in the illustrated provider configuration.

GitHub is shown as the concrete Git provider in the diagram, but Braid's lifecycle model is intended to remain independent of a particular provider.

## 1. Local Dev Box

The local development environment contains the agent integration, the only local command entry point, the recorder daemon, temporary event capture, the attached braid database, and local Git-note refs.
...
The Inbox is deliberately a staging boundary. Capturing an event does not by itself associate that event with a durable braid or thread.
...
This local database supports querying, retry, review preparation, note generation, and recovery without treating the Inbox as the durable braid model.
...
At the pull-request boundary, the worker drains the braid-related projection from Local SQLite to the remote authority. The synchronized envelope includes, as applicable:
...
```text
Before PR:
Inbox and Local SQLite are the local working truth.

At PR creation:
The attached braid projection is drained to Intent DB.

After PR creation:
Intent DB is the durable source of truth.

At merge:
Remaining events, the final projection, and merge SHA are drained before promotion is finalized.
```
...
The diagram uses GitHub to represent the provider boundary. Pull-request creation, pull-request updates, merge completion, and Git-note pushes are routed through the Local Braid Recorder Daemon rather than being performed directly by the Braid CLI.
...
This lets Braid record the action and its resulting provider identifiers as part of the same governed lifecycle.
...
9. Intent DB becomes the durable source of truth for that pull request.
...
12. Merge completion produces a `merge.completed` event and drains the remaining data plus merge SHA.
13. Promotion is finalized only against the accepted, current candidate.
14. The daemon generates the merge Git note and pushes the relevant note refs to the Git provider.
...
- Local SQLite is the working truth before the pull-request boundary.
- Intent DB is the durable truth after the pull-request boundary.
...
- Merge is the boundary that may finalize promotion; pull-request creation only makes work reviewable.
